...
def delete_ec2_amis_and_snapshots(client, username, retry_func):
    logger.info(f"Searching for EC2 AMIs with tag 'owner={username}'...")
    ...
    try:
        ...
        logger.warning(f"Failed to list tagged EC2 AMIs for owner '{username}': {e}")

    logger.info(f"Searching for EC2 snapshots with tag 'owner={username}'...")
    try:
        ...
        logger.warning(f"Failed to list tagged EC2 snapshots for owner '{username}': {e}")
